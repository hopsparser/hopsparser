...
class DatasetBPE:
    ...
    def to_bpe(self, sent_list, dataset_id):
        ...
        print(
            "%s applybpe /tmp/%s.bpe /tmp/%s %s"
            % (fastbpe, dataset_id, dataset_id, codes)
        )
        os.system(
            "%s applybpe /tmp/%s.bpe /tmp/%s %s"
            % (fastbpe, dataset_id, dataset_id, codes)
...
class DefaultLexer(nn.Module):
    ...
    def forward(self, word_sequence):
        xinput = torch.LongTensor(
            [
                self.stoi.get(elt, self.stoi[DefaultLexer.UNK_TOKEN])
                for elt in word_sequence
            ]
        )
...
class MultilingualLexer(nn.Module):
    ...
    def forward(self, bpe_sequence):
        ...
        bpe_sequence = [
            bpe_tok for bpe_tok in bpe_sequence if not bpe_tok.startswith("##")
        ]
...
class SelectiveBPELexer(nn.Module):
    ...
    def forward(self, bpe_string):
        ...
        sidxes = torch.LongTensor([self.dico.index(w) for w in bpe_sequence]).unsqueeze(
            dim=1
        )
        L = torch.LongTensor([len(bpe_sequence)])
        bpe_tensor = self.transformer(
            "fwd", x=sidxes, lengths=L, langs=None, causal=False
        ).contiguous()
        ...
        for (bpe_tok, bpe_vec) in zip(bpe_sequence, bpe_tensor):
            emb_buffer.append(
                bpe_vec
            )  # if crash here, check the bpe_embeddings_size of the model
