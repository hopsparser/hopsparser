...
#
# We also set a lower bound that's not too small, because nx overwrites (????) the original weigths
# in G with `weight + min_weight - 1 + (min_weight - max_weight)`, which clamps to 0 if the weight
# is something too small, like 2.225074e-308 or even 1e-16.
#
# TODO: report this as a bug to nx
...
@given(
    ...
    ).filter(lambda a: np.isfinite(np.linalg.norm(a.reshape(-1), ord=1)).all()),
)
