...
def test_trainer_train_parse(
    ...
    output_dir = tmp_path / "train_script_output"
    ret = script_runner.run(
        [
            "python",
            str(scripts_dir / "trainer.py"),
            str(train_config),
            str(treebank),
            "--dev-file",
            str(test_treebank),
    ...
    ret = script_runner.run(
        [
            "eval_parse",
            "-v",
            str(output_dir / f"{test_treebank.stem}.parsed.conllu"),
            str(test_treebank),
        ]
    )
    assert ret.success
    ...
    assert ret.success
    assert filecmp.cmp(
        output_dir / f"{test_treebank.stem}.parsed.conllu",
        tmp_path / f"{test_treebank.stem}.parsed2.conllu",
        shallow=False,
    )
    ret = script_runner.run(
